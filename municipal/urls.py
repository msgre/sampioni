...
from .views import ProgrammeListView, ProgrammeDetailView, ProgrammeItemDetailView,\
                   ProgrammeHTitleView, ProgrammeHDescriptionView
...
urlpatterns = patterns('',
    ...
    url(r'^(?P<year_from>[0-9]{4})-(?P<year_to>[0-9]{4})/(?P<programme>[\.0-9]+)/(?P<item>[\.0-9]+)/navrh-titulku/$', ProgrammeHTitleView.as_view(), name="programme-htitle"),
    url(r'^(?P<year_from>[0-9]{4})-(?P<year_to>[0-9]{4})/(?P<programme>[\.0-9]+)/(?P<item>[\.0-9]+)/navrh-popisu/$', ProgrammeHDescriptionView.as_view(), name="programme-hdescription"),
)
